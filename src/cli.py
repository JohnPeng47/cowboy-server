...
def drop_database(yes):
    ...
    if database_exists(str(config.SQLALCHEMY_DATABASE_URI)):
        ...
        click.secho(
            f"'{config.DATABASE_HOSTNAME}:{config.DATABASE_NAME}' does not exist!!!",
            fg="red",
        )
...
def upgrade_database(tag, sql, revision, revision_type):
    ...
    if not database_exists(str(config.SQLALCHEMY_DATABASE_URI)):
        ...
        if "dispatch_core" not in schema_names:
            ...
            conn.execute(
                sqlalchemy.text(open(config.ALEMBIC_MULTI_TENANT_MIGRATION_PATH).read())
            )
...
@click.option(
    "--exclude", multiple=True, help="Specifically exclude tasks you do no wish to run."
)
    ...
    if eager:
        for task in tasks:
            for r_task in scheduler.registered_tasks:
                ...
                click.secho(
                    f"A scheduled task/job named {task} does not exist", fg="red"
                )
...
def show_routes():
    ...
    click.secho(
        tabulate(table, headers=["Path", "Authenticated", "Methods"]), fg="blue"
    )
...
def run_server(log_level):
    ...
    if not os.path.isdir(config.STATIC_DIR):
        ...
        envvars.update(
            {x: getattr(config, x) for x in dir(config) if x.startswith("VITE_")}
        )
...
def _run_consume(
    plugin_slug: str, organization_slug: str, project_id: int, running: bool
):
